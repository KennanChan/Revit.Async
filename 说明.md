...
很可惜，答案是，并不是这样的，千万不要被名字里的"Async"误导了。
...
很多开发者产生“异步==多线程”这种误解的另一个原因在于，往往多线程变成都会伴随着异步编程。在大部分的UI应用开发中，当我们使用多线程执行一些数据获取、计算等任务时，我们希望将产生的结果“返回”到主线程上更新UI显示的内容，而这个“返回”正是异步发挥功能的地方。
...
如果你想要在一个后台线程中更新Revit模型，你需要使用`ExternalEvent`对象的`Raise()`方法把一个`IExternalEvent`发送到主线程排队执行，`IExternalEvent`里封装了Revit API的调用。而这也正是Revit提供的异步编程模型。
